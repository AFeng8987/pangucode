...
public class LitemallSystemConfigService {
    ...
    public Map<String, String> listCustomer() {
        LitemallSystemExample example = new LitemallSystemExample();
        example.or().andKeyNameLike("litemall_customer_%").andDeletedEqualTo(false);
        List<LitemallSystem> systemList = systemMapper.selectByExample(example);
        Map<String, String> data = new HashMap<>();
        for(LitemallSystem system : systemList){
            data.put(system.getKeyName(), system.getKeyValue());
    ...
    public String selByKey(String key) {
        LitemallSystemExample example = new LitemallSystemExample();
        example.or().andKeyNameEqualTo(key);
        LitemallSystem litemallSystem = systemMapper.selectOneByExample(example);
        return litemallSystem.getKeyValue();
    }

    public Map<String, String> hotNumber() {
        LitemallSystemExample example = new LitemallSystemExample();
        example.or().andKeyNameLike("litemall_wx_index_hot").andDeletedEqualTo(false);
        List<LitemallSystem> systemList = systemMapper.selectByExample(example);
        Map<String, String> data = new HashMap<>();
        for(LitemallSystem system : systemList){
            data.put(system.getKeyName(), system.getKeyValue());
