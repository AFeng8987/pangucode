...
public class OrderUtil {
    ...
    public static OrderHandleOption build(LitemallOrder order) {
        ...
        if (status == 101) {
            ...
            // 如果订单已经支付，且已经收货，则查物流、申请售后和再次购买
            handleOption.setLogistics(true);
            handleOption.setRebuy(true);
        } else if (status == 501) {
            // 如果订单已经过售后关闭，则可删除、和再次购买
            handleOption.setRebuy(true);
        } else {
    ...
    public static boolean isClose(LitemallOrder litemallOrder) {
        return OrderUtil.STATUS_CLOSE == litemallOrder.getOrderStatus().shortValue();
    }
