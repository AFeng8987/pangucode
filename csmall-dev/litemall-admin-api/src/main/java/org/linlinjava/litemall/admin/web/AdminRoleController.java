...
public class AdminRoleController {
    ...
    private Object validate(LitemallRole role) {
        ...
        if (StringUtils.isAnyBlank(name,desc)) {
            return ResponseUtil.badArgumentValue("必传参数不能为空");
        }
        if (!RegexUtil.isZh(name)||name.length()>10) {
            return ResponseUtil.badArgumentValue("角色名称不能超过10中文字符");
        }
        if (!RegexUtil.isZh(desc)||desc.length()>30) {
            return ResponseUtil.badArgumentValue("角色说明不能超过30中文字符");
        }
        return null;
    }
